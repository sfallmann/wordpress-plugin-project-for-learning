# Simple wordpress plugin using boilerplate for learning purposes
...
* A `spf-movie-content` directory that contains the source code - a fully executable WordPress plugin.
...
* `spf-movie-content/includes` is where functionality shared between the admin area and the public-facing parts of the site reside
* `spf-movie-content/admin` is for all admin-specific functionality
* `spf-movie-content/public` is for all public-facing functionality

Note that previous versions of the Boilerplate did not include `Spf_Movie_Content_Loader` but this class is used to register all filters and actions with WordPress.
